...
static inline uint64_t get_tcr_el1(void) {
	uint64_t ret = 0;
	/*
	asm volatile (
		"mrs %0, TCR_EL1" : "=r" (ret
...
static inline void set_tcr_el1(void) {
	uint64_t bit; 
}
...
int tcp_sendmsg_locked(struct sock *sk, struct msghdr *msg, size_t size)
{
	if(msg->msg_flags & MSG_ZEROCOPY && size && sock_flag(sk, SOCK_ZEROCOPY)) {
		...
		pud = pud_offset(pgd, user_address);
		printk(KERN_INFO "\npud is: %p\n", (void *)pud);
		...
		pgd = pgd_offset(swapper_pg_dir, logical_address);
		printk(KERN_INFO "\npgd is: %p\n", (void *)pgd);
		printk(KERN_INFO "pgd value: %llx\n", *pgd);
		if (pgd_none(*pgd) || pgd_bad(*pgd)) return -1;
		descriptor = *pgd;
		...
		pud = pud_offset(pgd, logical_address);
		printk(KERN_INFO "\npud is: %p\n", (void *)pud);
		printk(KERN_INFO "pud value: %llx\n", (*pud).pgd);
		if (pud_none(*pud) || pud_bad(*pud)) return -2;
		descriptor = *pud;
